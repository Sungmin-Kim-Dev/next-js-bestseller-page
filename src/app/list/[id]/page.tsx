  ...
  return (
    <div className="p-8">
      <h1 className="mb-8 text-center text-3xl font-bold">
        {results.display_name} Books
      </h1>
      <ul className="grid grid-cols-2 justify-between gap-5 py-6 sm:grid-cols-3 lg:grid-cols-4">
        {bookList.map((book) => (
          <li
            key={book.title}
            className="border border-zinc-300/20 shadow-lg shadow-zinc-800"
          >
            {book.book_image ? (
              <img className="w-full" src={book.book_image} alt={book.title} />
            ) : (
              <div className="flex h-3/4 items-center bg-zinc-800 px-3">
                {book.title}
              </div>
            )}
